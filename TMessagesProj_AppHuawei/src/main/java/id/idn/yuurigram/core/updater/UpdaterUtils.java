...
package id.idn.yuurigram.core.updater;
...
import id.idn.yuurigram.helpers.network.NetworkHelper;
...
public class UpdaterUtils {
    ...
    private static final String RELEASE_URI = "https://api.github.com/repos/arsLan4k1390/Yuurigram/releases/latest";
    private static final String BETA_URI = "https://api.github.com/repos/arsLan4k1390/YuurigramBeta-APKs/releases/latest";
    ...
    public static boolean updateFileExists() {
        version = YuurigramCoreConfig.INSTANCE.getUpdateVersionName();
        ...
        boolean isNew = cmp < 0;
        YuurigramCoreConfig.INSTANCE.setUpdateAvailable(isNew);
    ...
    public static void checkUpdates(BaseFragment fragment, boolean manual, OnUpdateNotFound onUpdateNotFound, OnUpdateFound onUpdateFound, Browser.Progress progress) {
        if (YuurigramCoreConfig.isStandalonePremiumBuild()) return;

        if (checkingForUpdates || id != 1L || (System.currentTimeMillis() - YuurigramCoreConfig.INSTANCE.getUpdateScheduleTimestamp() < updateCheckInterval && !manual))
            return;

        checkingForUpdates = true;
        otaQueue.postRunnable(() -> {
            YuurigramCoreConfig.INSTANCE.getLastUpdateCheckTime();
            YuurigramCoreConfig.INSTANCE.setLastUpdateCheckTime(System.currentTimeMillis());
            try {
                HttpURLConnection connection = (HttpURLConnection) new URI(RELEASE_URI).toURL().openConnection();
                if (YuurigramCoreConfig.INSTANCE.getInstallBetas()) {
                ...
                for (int i = 0; i < arr.length(); i++) {
                    ...
                    link = arr.getJSONObject(i).getString("browser_download_url")
                            .replace("Yuurigram-", "Yuurigram-Huawei-");
                    downloadURL = link;
                    if (YuurigramCoreConfig.isDevBuild()) FileLog.d("DownloadLinkHuawei: " + downloadURL);
                ...
                if (update.isNew() && fragment != null && fragment.getContext() != null) {
                    ...
                    YuurigramCoreConfig.INSTANCE.setUpdateIsDownloading(false);
                    YuurigramCoreConfig.INSTANCE.setUpdateAvailable(true);
                    if (version != null && !version.equals("0")) YuurigramCoreConfig.INSTANCE.setUpdateVersionName(version);
                    YuurigramCoreConfig.INSTANCE.setUpdateSize(size);
                } else {
    ...
    public static void downloadApk(Context context, String link, String title, ButtonWithCounterView progressTextView) {
        if (context != null && !updateDownloaded) {
            ...
            YuurigramCoreConfig.INSTANCE.setUpdateIsDownloading(true);
                    ...
                    if (cursor != null && cursor.moveToFirst()) {
                        ...
                        if (indexBytesDownloaded >= 0 && indexBytesTotal >= 0) {
                            ...
                            if (bytesTotal > 0) {
                                ...
                                YuurigramCoreConfig.INSTANCE.setUpdateDownloadingProgress(progress);
                                if (YuurigramCoreConfig.isDevBuild()) FileLog.e("Загрузка: " + progress + "%");
                            }
                        }
                    }
                } catch (Exception e) {
    ...
    public static void cancelDownload(Context context, long downloadId) {
        ...
        YuurigramCoreConfig.INSTANCE.setUpdateIsDownloading(false);
        YuurigramCoreConfig.INSTANCE.setUpdateDownloadingProgress(0f);
        NotificationCenter.getGlobalInstance().postNotificationName(NotificationCenter.appUpdateAvailable);
    }
    ...
    public static class Update {
        ...
        public boolean isNew() {
            ...
            YuurigramCoreConfig.INSTANCE.setUpdateAvailable(cmp < 0);
            return cmp < 0;
        }
    ...
    /*public static String getLastCheckUpdateTime() {
        return getString(R.string.UP_LastCheck) + ": " + LocaleController.formatDateTime(YuurigramCoreConfig.INSTANCE.getLastUpdateCheckTime() / 1000, true);
    }*/
