...
public class UpdaterBottomSheet extends BottomSheet {
    ...
    public UpdaterBottomSheet(Context context, Theme.ResourcesProvider resourcesProvider, boolean available, UpdaterUtils.Update update) {
        ...
        if (available) {
            ...
            downloadButton.setOnClickListener(v -> {
                if (!downloadButtonClicked) {
                    ...
                    UpdaterUtils.downloadApk(getContext(), update.downloadURL, "Yuurigram " + update.version, downloadButton);
                    dismiss();
                }
            });
            ...
            if (update.isForce()) {
                setCancelable(false);
                isForce = true;
                YuurigramCoreConfig.INSTANCE.setAutoOTA(true);
            }
            YuurigramCoreConfig.INSTANCE.setForceFound(update.isForce());
        } else {
            ...
            installBetas.setTextAndCheckAndIcon(getString(R.string.UP_InstallBetas), YuurigramCoreConfig.INSTANCE.getInstallBetas(), R.drawable.test_tube_solar, false);
            installBetas.setOnClickListener(v -> {
                YuurigramCoreConfig.INSTANCE.setInstallBetas(!YuurigramCoreConfig.INSTANCE.getInstallBetas());
            ...
            checkOnLaunch.setTextAndCheckAndIcon(getString(R.string.UP_Auto_OTA), YuurigramCoreConfig.INSTANCE.getAutoOTA(), R.drawable.msg_retry, false);
            checkOnLaunch.setOnClickListener(v -> {
                YuurigramCoreConfig.INSTANCE.setAutoOTA(!YuurigramCoreConfig.INSTANCE.getAutoOTA());
                checkOnLaunch.setChecked(!checkOnLaunch.isChecked());
            });
            if (!YuurigramCoreConfig.INSTANCE.getForceFound()) linearLayout.addView(checkOnLaunch);
        ...
        if (available && !isForce) {
            ...
            scheduleButton.setOnClickListener(v -> {
                YuurigramCoreConfig.INSTANCE.setUpdateScheduleTimestamp(System.currentTimeMillis());
                dismiss();
            });
    ...
    private void openGithubReleases() {
        String githubLink;
        if (YuurigramCoreConfig.isStandaloneBetaBuild()) {
