...
object YuurigramExtras : CoroutineScope by MainScope() {
    ...
    fun checkChannelFollow(activity: Activity, currentAccount: Int) = AndroidUtilities.runOnUIThread {
        ...
        if (updateChannel is TLRPC.Chat) {
            launch(Dispatchers.IO) {
                if (updateChannel.id != Constants.Yuurigram_Channel) {
            ...
            connectionsManager.sendRequest(TLRPC.TL_contacts_resolveUsername().apply {
                ...
                if (error == null) {
                    ...
                    launch(Dispatchers.IO) {
                        if (chat.id != Constants.Yuurigram_Channel) {
                            KotlinFragmentsManager.nfweioufwehr117()
                        }
    ...
    private fun checkChannelFollow(activity: Activity, currentAccount: Int, channel: TLRPC.Chat) {
        ...
        launch(Dispatchers.IO) {
            if (channel.id != Constants.Yuurigram_Channel) {
                KotlinFragmentsManager.nfweioufwehr117()
            }
...
//            builder.setNegativeButton(getString(R.string.Cancel), null)
    ...
    fun isEdgeToEdgeSupported(): Boolean {
        if (YuurigramCoreConfig.edgeToEdgeMode == YuurigramCoreConfig.EDGE_MODE_ENABLE) {
            return true
        } else {
            isEdgeToEdgeCached?.let { return it }

            if (Build.VERSION.SDK_INT < Build.VERSION_CODES.R || YuurigramCoreConfig.edgeToEdgeMode == YuurigramCoreConfig.EDGE_MODE_DISABLE) {
