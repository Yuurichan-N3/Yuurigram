...
public class DebugPreferencesEntry extends UniversalFragment {
    ...
    protected void fillItems(ArrayList<UItem> items, UniversalAdapter adapter) {
        ...
        if (!YuurigramCoreConfig.isStandaloneStableBuild() && !YuurigramCoreConfig.isPlayStoreBuild()) {
            items.add(SettingsHelper.asSwitchCG(toastRpcRow, "Toast all RPC errors *", "You'll see RPC errors from Telegram's backend as toast messages.")
                    .setChecked(YuurigramDebugConfig.INSTANCE.getShowRPCErrors())
            );
        }
        items.add(SettingsHelper.asSwitchCG(oldTimeStyleRow, "Default time style in chats *", "Unlike iOS and TDesktop")
                .setChecked(YuurigramDebugConfig.INSTANCE.getOldTimeStyle())
        );
        items.add(SettingsHelper.asSwitchCG(safeStarsRow, "Use SafeStars *")
                .setChecked(YuurigramCoreConfig.INSTANCE.getAllowSafeStars())
        );
        ...
        items.add(SettingsHelper.asSwitchCG(replacePunctuationRow, "Replace punctuation marks *", "Replace quotation marks and dashes like on TDesktop")
                .setChecked(YuurigramDebugConfig.INSTANCE.getReplacePunctuationMarks())
        );
        items.add(SettingsHelper.asSwitchCG(editTextFixRow, "EditTextSugestionsFix *", "Emojis/formatting disappear when Samsung puts suggestions in edit")
                .setChecked(YuurigramDebugConfig.INSTANCE.getEditTextSuggestionsFix())
        );
        ...
        items.add(SettingsHelper.asSwitchCG(sendMaxQualityRow, "Send videos at max quality *", "Max quality will be automatically selected when you send a video")
                .setChecked(YuurigramDebugConfig.INSTANCE.getSendVideosAtMaxQuality())
        );
        items.add(SettingsHelper.asSwitchCG(playGifAsVideoRow, "Play GIFs as Videos *")
                .setChecked(YuurigramDebugConfig.INSTANCE.getPlayGIFsAsVideos())
        );
        items.add(SettingsHelper.asSwitchCG(hideTimestampRow, "Hide video timestamp *", "Saved progress for videos. Return exactly where you left off.")
                .setChecked(YuurigramDebugConfig.INSTANCE.getHideVideoTimestamp())
        );
        ...
        items.add(UItem.asShadow("* Yuurigram's feature."));
        items.add(UItem.asShadow(null));
    }
    ...
    protected void onClick(UItem item, View view, int position, float x, float y) {
        if (item.id == toastRpcRow) {
            YuurigramDebugConfig.INSTANCE.setShowRPCErrors(!YuurigramDebugConfig.INSTANCE.getShowRPCErrors());
            SettingsHelper.updateCheckState(view, YuurigramDebugConfig.INSTANCE.getShowRPCErrors());

            CGBulletinCreator.INSTANCE.createRestartBulletin(this);
        } else if (item.id == oldTimeStyleRow) {
            YuurigramDebugConfig.INSTANCE.setOldTimeStyle(!YuurigramDebugConfig.INSTANCE.getOldTimeStyle());
            SettingsHelper.updateCheckState(view, YuurigramDebugConfig.INSTANCE.getOldTimeStyle());
        } else if (item.id == safeStarsRow) {
            YuurigramCoreConfig.INSTANCE.setAllowSafeStars(!YuurigramCoreConfig.INSTANCE.getAllowSafeStars());
            SettingsHelper.updateCheckState(view, YuurigramCoreConfig.INSTANCE.getAllowSafeStars());
            ...
            YuurigramDebugConfig.INSTANCE.setReplacePunctuationMarks(!YuurigramDebugConfig.INSTANCE.getReplacePunctuationMarks());
            SettingsHelper.updateCheckState(view, YuurigramDebugConfig.INSTANCE.getReplacePunctuationMarks());

            CGBulletinCreator.INSTANCE.createRestartBulletin(this);
        } else if (item.id == editTextFixRow) {
            YuurigramDebugConfig.INSTANCE.setEditTextSuggestionsFix(!YuurigramDebugConfig.INSTANCE.getEditTextSuggestionsFix());
            SettingsHelper.updateCheckState(view, YuurigramDebugConfig.INSTANCE.getEditTextSuggestionsFix());
            ...
            YuurigramDebugConfig.INSTANCE.setSendVideosAtMaxQuality(!YuurigramDebugConfig.INSTANCE.getSendVideosAtMaxQuality());
            SettingsHelper.updateCheckState(view, YuurigramDebugConfig.INSTANCE.getSendVideosAtMaxQuality());
        } else if (item.id == playGifAsVideoRow) {
            YuurigramDebugConfig.INSTANCE.setPlayGIFsAsVideos(!YuurigramDebugConfig.INSTANCE.getPlayGIFsAsVideos());
            SettingsHelper.updateCheckState(view, YuurigramDebugConfig.INSTANCE.getPlayGIFsAsVideos());
        } else if (item.id == hideTimestampRow) {
            YuurigramDebugConfig.INSTANCE.setHideVideoTimestamp(!YuurigramDebugConfig.INSTANCE.getHideVideoTimestamp());
            SettingsHelper.updateCheckState(view, YuurigramDebugConfig.INSTANCE.getHideVideoTimestamp());
    ...
    private void showAudioSourceDialog(Runnable runnable) {
        ...
        configValues.add(YuurigramDebugConfig.AUDIO_SOURCE_DEFAULT);

        configStringKeys.add("CAMCORDER");
        configValues.add(YuurigramDebugConfig.AUDIO_SOURCE_CAMCORDER);

        configStringKeys.add("MIC");
        configValues.add(YuurigramDebugConfig.AUDIO_SOURCE_MIC);

        configStringKeys.add("REMOTE_SUBMIX");
        configValues.add(YuurigramDebugConfig.AUDIO_SOURCE_REMOTE_SUBMIX);

        configStringKeys.add("UNPROCESSED");
        configValues.add(YuurigramDebugConfig.AUDIO_SOURCE_UNPROCESSED);

        configStringKeys.add("VOICE_CALL");
        configValues.add(YuurigramDebugConfig.AUDIO_SOURCE_VOICE_CALL);

        configStringKeys.add("VOICE_COMMUNICATION");
        configValues.add(YuurigramDebugConfig.AUDIO_SOURCE_VOICE_COMMUNICATION);

        configStringKeys.add("VOICE_DOWNLINK");
        configValues.add(YuurigramDebugConfig.AUDIO_SOURCE_VOICE_DOWNLINK);

        configStringKeys.add("VOICE_PERFORMANCE");
        configValues.add(YuurigramDebugConfig.AUDIO_SOURCE_VOICE_PERFORMANCE);

        configStringKeys.add("VOICE_RECOGNITION");
        configValues.add(YuurigramDebugConfig.AUDIO_SOURCE_VOICE_RECOGNITION);

        configStringKeys.add("VOICE_UPLINK");
        configValues.add(YuurigramDebugConfig.AUDIO_SOURCE_VOICE_UPLINK);

        PopupHelper.show(configStringKeys, "Microphone Audio Source *", configValues.indexOf(YuurigramDebugConfig.INSTANCE.getAudioSource()), getContext(), i -> {
            YuurigramDebugConfig.INSTANCE.setAudioSource(configValues.get(i));
            if (runnable != null) runnable.run();
        });
    }

    private String getAudioSourceValue() {
        return switch (YuurigramDebugConfig.INSTANCE.getAudioSource()) {
            case YuurigramDebugConfig.AUDIO_SOURCE_CAMCORDER -> "CAMCORDER";
            case YuurigramDebugConfig.AUDIO_SOURCE_MIC -> "MIC";
            case YuurigramDebugConfig.AUDIO_SOURCE_REMOTE_SUBMIX -> "REMOTE_SUBMIX";
            case YuurigramDebugConfig.AUDIO_SOURCE_UNPROCESSED -> "UNPROCESSED";
            case YuurigramDebugConfig.AUDIO_SOURCE_VOICE_CALL -> "VOICE_CALL";
            case YuurigramDebugConfig.AUDIO_SOURCE_VOICE_COMMUNICATION -> "VOICE_COMMUNICATION";
            case YuurigramDebugConfig.AUDIO_SOURCE_VOICE_DOWNLINK -> "VOICE_DOWNLINK";
            case YuurigramDebugConfig.AUDIO_SOURCE_VOICE_PERFORMANCE -> "VOICE_PERFORMANCE";
            case YuurigramDebugConfig.AUDIO_SOURCE_VOICE_RECOGNITION -> "VOICE_RECOGNITION";
            case YuurigramDebugConfig.AUDIO_SOURCE_VOICE_UPLINK -> "VOICE_UPLINK";
            default -> "DEFAULT";
        };
    }
