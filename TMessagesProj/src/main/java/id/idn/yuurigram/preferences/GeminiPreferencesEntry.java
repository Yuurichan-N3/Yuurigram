...
import id.idn.yuurigram.chats.gemini.network.ApiClient;
import id.idn.yuurigram.chats.gemini.network.ModelInfo;
...
public class GeminiPreferencesEntry extends UniversalFragment {
    ...
    protected void fillItems(ArrayList<UItem> items, UniversalAdapter adapter) {
        ...
        if (!TextUtils.isEmpty(YuurigramMessagesConfig.INSTANCE.getGeminiApiKey())) {
            geminiApiKeyField.getEditText().setText(YuurigramMessagesConfig.INSTANCE.getGeminiApiKey());
        }
        ...
        geminiModelNameField.getEditText().setText(YuurigramMessagesConfig.INSTANCE.getGeminiModelName());
        ...
        geminiSystemPromptField.getEditText().setText(YuurigramMessagesConfig.INSTANCE.getGeminiSystemPrompt());
        ...
        items.add(
                UItem.asIntSlideView(
                        1,
                        1,
                        YuurigramMessagesConfig.INSTANCE.getGeminiTemperatureValue(),
                        10,
                        val -> String.format(Locale.US, "%.1f", val / 10f),
                        YuurigramMessagesConfig.INSTANCE::setGeminiTemperatureValue
                )
        );
    ...
    protected void onClick(UItem item, View view, int position, float x, float y) {
        if (item.id == geminiModelsListButton) {
            ApiClient.fetchModels(
                    getContext(),
                    getResourceProvider(),
                    YuurigramMessagesConfig.INSTANCE.getGeminiApiKey(),
                    models -> AndroidUtilities.runOnUIThread(() -> {
                        ...
                        PopupHelper.show(getString(R.string.CP_GeminiAI_Model), modelFullTitleArray, modelDescriptionArray, modelShortTitleArray.indexOf(YuurigramMessagesConfig.INSTANCE.getGeminiModelName()), getContext(), i -> {
                            YuurigramMessagesConfig.INSTANCE.setGeminiModelName(modelShortTitleArray.get(i));
    ...
    private void doOnDone() {
        ...
        YuurigramMessagesConfig.INSTANCE.setGeminiApiKey(
        ...
        YuurigramMessagesConfig.INSTANCE.setGeminiModelName(
                geminiModelNameField.getEditText().getText().toString()
        );

        YuurigramMessagesConfig.INSTANCE.setGeminiSystemPrompt(
