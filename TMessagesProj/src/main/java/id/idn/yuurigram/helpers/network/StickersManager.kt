...
object StickersManager {
    ...
    private const val GITLAB_RAW_URL = "https://gitlab.com/arsLan4k1390/Yuurigram-IDS/-/raw/main/stickers.txt?inline=false"
    private val REFRESH_INTERVAL = if (YuurigramCoreConfig.isDevBuild()) 60 * 60 * 1000L else 24 * 60 * 60 * 1000L // 24 hours

    private val blockedStickersIds = mutableSetOf<Long>()

    suspend fun startAutoRefresh(context: Context) {
        val lastUpdateTime = YuurigramChatsConfig.lastStickersCheckTime
    ...
    private suspend fun updaterStickersList(context: Context) {
        withContext(Dispatchers.IO) {
            try {
                ...
                YuurigramChatsConfig.lastStickersCheckTime = System.currentTimeMillis()
