...
object CGChatMenuInjector {
    ...
    fun injectYuurigramShortcuts(
        ...
        val requireBiometrics = YuurigramPrivacyConfig.askBiometricsToOpenChat && !secretChat
        val isAnyButtonEnabled = requireBiometrics || YuurigramChatsConfig.shortcut_JumpToBegin
                    || YuurigramChatsConfig.shortcut_DeleteAll || YuurigramChatsConfig.shortcut_SavedMessages
                    || YuurigramChatsConfig.shortcut_Browser
        ...
        if (YuurigramChatsConfig.shortcut_JumpToBegin) headerItem.lazilyAddSubItem(
        ...
        if (currentChat != null && !isDeleteAllHidden(currentChat) && (ChatObject.isMegagroup(currentChat) || !ChatObject.isChannel(currentChat))) {
            if (YuurigramChatsConfig.shortcut_DeleteAll) headerItem.lazilyAddSubItem(
        ...
        if (currentChat != null && currentChat.id != abs(ChatsHelper2.getCustomChatID())
            ...
            if (YuurigramChatsConfig.shortcut_SavedMessages) headerItem.lazilyAddSubItem(
        ...
        if (YuurigramChatsConfig.shortcut_Browser) headerItem.lazilyAddSubItem(
    ...
    fun injectAdminShortcuts(headerItem: ActionBarMenuItem, currentChat: TLRPC.Chat) {
        val isAnyButtonEnabled = YuurigramChatsConfig.admins_Reactions || YuurigramChatsConfig.admins_Permissions || YuurigramChatsConfig.admins_Administrators
                || YuurigramChatsConfig.admins_Members || YuurigramChatsConfig.admins_Statistics || YuurigramChatsConfig.admins_RecentActions

        if (isAnyButtonEnabled) headerItem.lazilyAddColoredGap()

        if (YuurigramChatsConfig.admins_Reactions && ChatObject.canChangeChatInfo(currentChat)) headerItem.lazilyAddSubItem(
                ChatActivityHelper.OPTION_FOR_ADMINS_REACTIONS,
                R.drawable.msg_reactions2,
                getString(R.string.Reactions)
        )

        if (YuurigramChatsConfig.admins_Permissions && !(ChatObject.isChannel(currentChat) && !currentChat.megagroup) && !currentChat.gigagroup) headerItem.lazilyAddSubItem(
                ChatActivityHelper.OPTION_FOR_ADMINS_PERMISSIONS,
                R.drawable.msg_permissions,
                getString(R.string.ChannelPermissions)
        )

        if (YuurigramChatsConfig.admins_Administrators) headerItem.lazilyAddSubItem(
                ChatActivityHelper.OPTION_FOR_ADMINS_ADMINISTRATORS,
                R.drawable.msg_admins,
                getString(R.string.ChannelAdministrators)
        )

        if (YuurigramChatsConfig.admins_Members) headerItem.lazilyAddSubItem(
        ...
        if (YuurigramChatsConfig.admins_Statistics && ChatObject.isBoostSupported(currentChat)) headerItem.lazilyAddSubItem(
                ChatActivityHelper.OPTION_FOR_ADMINS_STATISTICS,
                R.drawable.msg_stats,
                getString(R.string.StatisticsAndBoosts)
        )

        if (YuurigramChatsConfig.admins_RecentActions) headerItem.lazilyAddSubItem(
    ...
    private fun isDeleteAllHidden(chat: TLRPC.Chat): Boolean {
        return Constants.Yuurigram_Support == chat.id
    }
    ...
    fun injectArchived(io: ItemOptions, fragment: BaseFragment) {
        io.addIf(
            !YuurigramPrivacyConfig.hideArchiveFromChatsList && (YuurigramAppearanceConfig.tabsHideAllChats || !MainTabsManager.hasTab(MainTabsManager.TabType.SETTINGS) || !YuurigramAppearanceConfig.showMainTabs),
    ...
    fun injectSaved(io: ItemOptions, fragment: BaseFragment) {
        io.addIf(
            !MainTabsManager.hasTab(MainTabsManager.TabType.SETTINGS) || !YuurigramAppearanceConfig.showMainTabs,
            R.drawable.msg_saved,
            getString(R.string.SavedMessages)
        ) {
    ...
    fun injectCalls(io: ItemOptions, fragment: BaseFragment) {
        io.addIf(
            !YuurigramAppearanceConfig.showMainTabs || !MainTabsManager.hasTab(MainTabsManager.TabType.CALLS),
    ...
    fun injectGifts(io: ItemOptions, currentAccount: Int, context: Context) {
        val available: Boolean = YuurigramAppearanceConfig.marketPlaceDrawerButton && DonatesManager.checkAllDonatedAccountsForMarketplace()
