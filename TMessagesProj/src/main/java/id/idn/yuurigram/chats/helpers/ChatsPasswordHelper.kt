...
class ChatsPasswordHelper private constructor(num: Int) : BaseController(num) {
    ...
    fun saveArrayList(list: ArrayList<String>, key: String) {
        if (YuurigramCoreConfig.isDevBuild()) FileLog.d("запросил saveArrayList")
    ...
    fun getArrayList(key: String): ArrayList<String> {
        if (YuurigramCoreConfig.isDevBuild()) FileLog.d("запросил кешированный getArrayList для паролей")

        if (key == getPasscodeArray() && lockedChatsCache != null) {
            return ArrayList(lockedChatsCache!!)
        }

        if (YuurigramCoreConfig.isDevBuild()) FileLog.d("запросил getArrayList для паролей")

        val json = messagesController.mainSettings.getString(key, null)
        if (YuurigramCoreConfig.isDevBuild()) FileLog.d("getArrayList: $json")
    ...
    fun isChatLocked(chatId: Long): Boolean {
        if (YuurigramCoreConfig.isDevBuild()) FileLog.d("запросил isChatLocked")
        if (chatId == 0L || !YuurigramPrivacyConfig.askBiometricsToOpenChat) return false
    ...
    fun isChatLocked(messageObject: MessageObject): Boolean {
        if (YuurigramCoreConfig.isDevBuild()) FileLog.d("запросил isChatLocked2")
        return YuurigramPrivacyConfig.askBiometricsToOpenChat && messageObject.messageOwner.message != null
    ...
    fun isEncryptedChat(chatId: Long): Boolean {
        if (YuurigramCoreConfig.isDevBuild()) FileLog.d("запросил isEncryptedChat")
        if (YuurigramPrivacyConfig.askBiometricsToOpenEncrypted) {
    ...
    fun isEncryptedChat(messageObject: MessageObject): Boolean {
        if (YuurigramCoreConfig.isDevBuild()) FileLog.d("запросил isEncryptedChat2")
        if (YuurigramPrivacyConfig.askBiometricsToOpenEncrypted) {
    ...
    fun checkLockedChatsEntities(messageObject: MessageObject): ArrayList<MessageEntity>? {
        if (YuurigramCoreConfig.isDevBuild()) FileLog.d("запросил checkLockedChatsEntities")
    ...
    fun checkLockedChatsEntities(messageObject: MessageObject, original: ArrayList<MessageEntity>?): ArrayList<MessageEntity>? {
        if (YuurigramCoreConfig.isDevBuild()) FileLog.d("запросил checkLockedChatsEntities2")
    ...
    fun replaceStringToSpoilers(originalText: String?, force: Boolean): String? {
        if (YuurigramCoreConfig.isDevBuild()) FileLog.d("запросил replaceStringToSpoilers")
        if (originalText == null) {
            return null
        }
        return if (YuurigramPrivacyConfig.askBiometricsToOpenArchive || force) {
    ...
    fun getLockedChatsCount(): Int {
        if (YuurigramCoreConfig.isDevBuild()) FileLog.d("запросил getLockedChatsCount")
        return getArrayList(getPasscodeArray()).size
    }

    fun shouldRequireBiometrics(userID: Long, chatID: Long, encID: Long): Boolean {
        if (YuurigramCoreConfig.isDevBuild()) FileLog.d("запросил shouldRequireBiometrics")
    ...
    fun shouldRequireBiometricsToOpenChats(): Boolean {
        if (YuurigramCoreConfig.isDevBuild()) FileLog.d("запросил shouldRequireBiometricsToOpenChats")
        return YuurigramPrivacyConfig.askBiometricsToOpenChat && checkBiometricAvailable()
    }

    fun shouldRequireBiometricsToOpenEncryptedChats(): Boolean {
        if (YuurigramCoreConfig.isDevBuild()) FileLog.d("запросил shouldRequireBiometricsToOpenEncryptedChats")
        return YuurigramPrivacyConfig.askBiometricsToOpenEncrypted && checkBiometricAvailable()
    }

    fun askPasscodeBeforeDelete(): Boolean {
        if (YuurigramCoreConfig.isDevBuild()) FileLog.d("запросил askPasscodeBeforeDelete")
        return YuurigramPrivacyConfig.askPasscodeBeforeDelete && checkBiometricAvailable()
    }

    fun checkBiometricAvailable(): Boolean {
        if (YuurigramCoreConfig.isDevBuild()) FileLog.d("запросил checkBiometricAvailable")
