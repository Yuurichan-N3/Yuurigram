...
public class Translator {
    ...
    public static void showTranslationTargetSelector(Context context, boolean isKeyboard, Runnable callback, Theme.ResourcesProvider resourcesProvider) {
        ...
        PopupHelper.show(names, getString(R.string.CG_TranslationLanguage), targetLanguages.indexOf(isKeyboard ? YuurigramMessagesConfig.INSTANCE.getTranslationKeyboardTarget() : YuurigramMessagesConfig.INSTANCE.getTranslationTarget()), context, i -> {
            if (isKeyboard) {
                YuurigramMessagesConfig.INSTANCE.setTranslationKeyboardTarget(targetLanguages.get(i));
            } else {
                YuurigramMessagesConfig.INSTANCE.setTranslationTarget(targetLanguages.get(i));
            }
            callback.run();
        }, resourcesProvider);
    }
